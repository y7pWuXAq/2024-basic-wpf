    ...
    public class MainViewModel : Conductor<object>
    {
        ...
        public int Id
        {
            ...
            set
            {
                id = value;
                NotifyOfPropertyChange(() => Id);
                NotifyOfPropertyChange(() => CanDelEmployee); // 삭제여부 솏ㅇ도 변경 되었음을 공지
            }
        }
        ...
        // DataGrid에 띄울 Employees 테이블 데이터
        ...
        public void SaveEmployee()
        {
            ...
            using (SqlConnection conn = new SqlConnection(Helpers.Common.CONNSTRING))
            {
                ...
                if (Id == 0) // INSERT
                    ...
                    cmd.CommandText = Models.Employees.UPDEATE_QUERY;
                ...
                if (result > 0)
                {
                    MessageBox.Show("저장 완료 ^^7");
                }
                else
                {
                    MessageBox.Show("저장 실패 T^T");
                }
        ...
        {
            get { return Id != 0; } // TextBox Id 속성의 값이 0 false, 0이 아니면 true
        }
        ...
        public void DelEmployee()
        {
            if (Id == 0)
            {
                MessageBox.Show("삭제 멈춰!");
                return;
            }

            if (MessageBox.Show("삭제하시겠습니까?", "경고", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.No)
            {
                return;
            }
            ...
            using (SqlConnection conn = new SqlConnection(Helpers.Common.CONNSTRING))
            {
                ...
                if (res > 0)
                {
                    MessageBox.Show("삭제 완료 ^^7");
                }
                else
                {
                    MessageBox.Show("삭제 실패 T^T");
                }
